...
# Define tracked cities as a strict Enum dropdown
class TrackedCity(str, Enum):
    hyderabad = "Hyderabad"
    bengaluru = "Bengaluru"
    chennai = "Chennai"
    delhi = "Delhi"
...
def get_city_aqi(city: TrackedCity, limit: int = 10):
    query = text("""
        SELECT city, aqi, pm2_5, pm10, recorded_at 
        FROM aqi_readings 
    ...
    """)
    with engine.connect() as conn:
        result = conn.execute(query, {"city": city.value, "limit": limit})
        rows = result.fetchall()
        
        if not rows:
            raise HTTPException(status_code=404, detail=f"No data found for city: {city.value}")
    ...
    return {"city": city.value, "count": len(data), "records": data}